...
t.test('bin in local pkg', async t => {
  ...
  // now link a bin which will conflict with the one we try to run next. we have
  // to manually reset the bin links cache otherwise the code we are testing
  // wont run. in a real world situation these would happen during different
  ...
  await t.rejects(() => exec({ localBin, args: ['b'] }))
})
