...
const hostedFromURL = require('hosted-git-info').fromUrl
...
const addLocal = require('./add-local.js')
const correctMkdir = require('../utils/correct-mkdir.js')
const git = require('../utils/git.js')
const npm = require('../npm.js')
const rm = require('../utils/gently-rm.js')
const tempFilename = require('../utils/temp-filename.js')

const remotes = path.resolve(npm.config.get('cache'), '_git-remotes')
const templates = path.join(remotes, '_templates')
...
function tryHTTPS (from, hostedInfo, cb) {
  const httpsURL = hostedInfo.https()
  if (!httpsURL) {
...
function tryClone (from, combinedURL, silent, cb) {
  ...
  const normalized = normalizeGitUrl(combinedURL)
  const cloneURL = normalized.url
  const treeish = normalized.branch
  ...
  const repoID = path.relative(remotes, cachedRemote)
...
function resolveHead (from, cloneURL, treeish, cachedRemote, cb) {
  ...
  const args = ['rev-list', '-n1', treeish]
  git.whichAndExec(
    args,
    { cwd: cachedRemote, env: gitEnv() },
    function (er, stdout, stderr) {
      ...
      const resolvedTreeish = stdout.trim()
      log.silly('resolveHead', from, 'resolved treeish:', resolvedTreeish)

      const resolvedURL = getResolved(cloneURL, resolvedTreeish)
      if (!resolvedURL) {
        return cb(new Error(
      ...
      const tmpdir = path.join(tempFilename('git-cache'), resolvedTreeish)
...
function cloneResolved (from, resolvedURL, resolvedTreeish, cachedRemote, tmpdir, cb) {
  const args = ['clone', cachedRemote, tmpdir]
  git.whichAndExec(
    args,
    { cwd: cachedRemote, env: gitEnv() },
    function (er, stdout, stderr) {
...
function checkoutTreeish (from, resolvedURL, resolvedTreeish, tmpdir, cb) {
  const args = ['checkout', resolvedTreeish]
  git.whichAndExec(
    args,
...
function updateSubmodules (from, resolvedURL, tmpdir, cb) {
  const args = ['submodule', '-q', 'update', '--init', '--recursive']
  git.whichAndExec(
    args,
    { cwd: tmpdir, env: gitEnv() },
    function (er, stdout, stderr) {
...
let gitEnv_
function gitEnv () {
  // git responds to env consts in some weird ways in post-receive hooks
...
function getResolved (uri, treeish) {
  ...
  const rehydrated = hostedFromURL(uri)
  if (rehydrated) {
