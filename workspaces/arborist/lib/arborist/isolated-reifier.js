...
// Sanitize a package name for safe use in filesystem paths.
// Rejects path traversal sequences that could escape the store directory.
...
module.exports = (cls) =>
  class IsolatedReifier extends cls {
    ...
    async makeIdealGraph() {
      ...
      this.idealGraph.workspaces = await Promise.all(
        Array.from(idealTree.fsChildren.values(), (w) =>
          this.#workspaceProxy(w),
        ),
      );
      ...
      while (queue.length !== 0) {
        ...
        next.edgesOut.forEach((edge) => {
          if (
            edge.to &&
            !(
              next.package.bundleDependencies ||
              next.package.bundledDependencies ||
              []
            ).includes(edge.to.name) &&
            !edge.to.shouldOmit?.(omit)
          ) {
            queue.push(edge.to);
          }
        });
        ...
        if (
          ...
          !next.isWorkspace &&
          !next.inert &&
          !idealTree.fsChildren.has(next) &&
          !idealTree.fsChildren.has(next.target)
        ) {
    ...
    async #assignCommonProperties(node, result, populateDeps = true) {
      ...
      result.name = result.isWorkspace
      ...
      const edges = [...node.edgesOut.values()].filter(
        (edge) =>
          edge.to?.target &&
          !(
            node.package.bundledDependencies || node.package.bundleDependencies
          )?.includes(edge.to.name),
      );
      const nonOptionalDeps = edges
        .filter((edge) => !edge.optional)
        .map((edge) => edge.to.target);
      ...
      const optionalDeps = edges
        .filter((edge) => edge.optional)
        .map((edge) => edge.to.target);
      result.localDependencies = await Promise.all(
        nonOptionalDeps.filter(isLocal).map((n) => this.#workspaceProxy(n)),
      );
      result.externalDependencies = await Promise.all(
        nonOptionalDeps
          .filter((n) => !isLocal(n) && !n.inert)
          .map((n) => this.#externalProxy(n)),
      );
      result.externalOptionalDependencies = await Promise.all(
        optionalDeps.filter((n) => !n.inert).map((n) => this.#externalProxy(n)),
      );
      result.dependencies = [
        ...result.externalDependencies,
        ...result.localDependencies,
    ...
    async #createBundledTree() {
      ...
      for (const [, edge] of idealTree.edgesOut) {
        if (
          edge.to &&
          (
            idealTree.package.bundleDependencies ||
            idealTree.package.bundledDependencies ||
            []
          ).includes(edge.to.name)
        ) {
      ...
      for (const child of idealTree.fsChildren) {
        for (const [, edge] of child.edgesOut) {
          if (
            edge.to &&
            (
              child.package.bundleDependencies ||
              child.package.bundledDependencies ||
              []
            ).includes(edge.to.name)
          ) {
      ...
      while (queue.length !== 0) {
        ...
        if (!from.isRoot && !from.isWorkspace) {
          nodes.set(from.location, {
            location: from.location,
            resolved: from.resolved,
            name: from.name,
            optional: from.optional,
            pkg: { ...from.package, bundleDependencies: undefined },
          });
        }
        const to = nextEdge.to;
        nodes.set(to.location, {
          location: to.location,
          resolved: to.resolved,
          name: to.name,
          optional: to.optional,
          pkg: { ...to.package, bundleDependencies: undefined },
        });
        edges.push({
          from: from.isRoot ? "root" : from.location,
          to: to.location,
        });
    ...
    async createIsolatedTree() {
      ...
      bundledTree.edges.forEach((edge) => {
        const from =
          edge.from === "root" ? root : root.inventory.get(edge.from);
    ...
    #processEdges(node, externalEdge, root) {
      ...
      let from, nmFolder;
      if (externalEdge) {
        const fromLocation = join(
          "node_modules",
          ".store",
          key,
          "node_modules",
          sanitizeName(node.packageName),
        );
        from = root.children.get(fromLocation);
        nmFolder = join("node_modules", ".store", key, "node_modules");
      } else {
        from = node.isProjectRoot
          ? root
          : root.inventory.get(node.localLocation);
    ...
    #processDeps(dep, optional, external, root, from, nmFolder) {
      ...
      let target;
      if (external) {
        const toLocation = join(
          "node_modules",
          ".store",
          toKey,
          "node_modules",
          sanitizeName(dep.packageName),
        );
      ...
      const link = new IsolatedLink({
        ...
        resolved: external
          ? `file:.store/${toKey}/node_modules/${sanitizeName(dep.packageName)}`
          : dep.resolved,
        root,
        target,
      });
