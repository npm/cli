// XXX test/shrinkwrap.js tests fail if this is not first. Something weird is going on here
const { readdirScoped } = require('@npmcli/fs')

// mixin implementing the buildIdealTree, loadVirtual, and loadActual methods
...
// note: some of these symbols are shared so we can hit them with unit tests and reuse them across mixins
...
module.exports = cls => class IdealTreeBuilder extends cls {
  #actualTree
  // ensure when walking the tree that we don't call loadTree on the same actual node more than one time.
  #actualTreeLoaded = new Set()
  #actualTreePromise
  // cache of nodes when loading the actualTree, so that we avoid loaded the same node multiple times when symlinks attack.
  #cache = new Map()
  #complete
  #currentDep = null
  ...
  // And, we know that it can ONLY be relevant when the node is a target of a link; otherwise, it'd be in a node_modules folder, so take advantage of that to limit the scans later.
  #topNodes = new Set()
  #transplantFilter
...
`)
  ...
  async loadVirtual (options = {}) {
    ...
    // allow the user to set reify options on the ctor as well.
    // XXX: deprecate separate instance method options object.
    options = { ...this.options, ...options }
    ...
    // when building the ideal tree, we pass in a root node to this function otherwise, load it from the root package json or the lockfile
    const pkg = await PackageJson.normalize(this.path).then(p => p.content).catch(() => s.data.packages[''] || {})
    // TODO clean this up
    const {
      root = await this[_setWorkspaces](this.#loadNode('', pkg, true)),
    } = options
