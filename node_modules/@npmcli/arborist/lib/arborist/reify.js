...
module.exports = cls => class Reifier extends cls {
  ...
  async reify (options = {}) {
    ...
    await this[_loadTrees](options)
      ...
      .then(() => this.log.warn('START REIFY PACKAGES'))
      .then(() => this[_reifyPackages]())
      .then(() => this.log.warn('FINISH REIFY PACKAGES'))
      .then(() => this[_saveIdealTree](options))
      .then(() => this[_copyIdealToActual]())
      .then(() => process.emit('time', 'reify:awaitQuickAudit'))
      .then(() => this.log.warn('START await quick audit'))
      .then(() => this[_awaitQuickAudit]())
      .then(() => this.log.warn('FINISH await quick audit'))
      .then(() => process.emit('timeEnd', 'reify:awaitQuickAudit'))
  ...
  async [_reifyPackages] () {
    this.log.warn('REIFY PACKAGES', {
      dryRun: this[_dryRun],
      packageLockOnly: this[_packageLockOnly],
    })
    ...
    //this.log.warn('1. RETIRE')
    await this[_retireShallowNodes]()
    //this.log.warn('2. CREATE SPARSE')
    await this[_createSparseTree]()
    //this.log.warn('3. OMITS TO TRASH LIST')
    await this[_addOmitsToTrashList]()
    //this.log.warn('4. LOAD SHRINKWRAPS AND UPDATE TREES')
    await this[_loadShrinkwrapsAndUpdateTrees]()
    //this.log.warn('5. LOAD BUNDLES AND UPDATE TREES')
    await this[_loadBundlesAndUpdateTrees]()
    //this.log.warn('6. SUBMIT QUICK AUDIT')
    await this[_submitQuickAudit]()
    this.log.warn('7. UNPACK NEW MODULES')
    await this[_unpackNewModules]()
    this.log.warn('8. MOVE BACK RETIRED UNCHANGED')
    await this[_moveBackRetiredUnchanged]()
    this.log.warn('9. BUILD')
    await this[_build]()
    this.log.warn('10. REMOVE TRASH')
    await this[_removeTrash]()
    this.log.warn('FINISH REIFY PACKAGES')
  }
  ...
  [_rollbackCreateSparseTree] (er) {
    this.log.warn('ROLL BACK CREATE SPARSE TREE', er)
  ...
  [_reifyNode] (node) {
    if (this[_trashList].has(node.path)) {
      console.error('ALREADY ON TRASH LIST', node.path)
      return node
    }

    const log = this._REIFY_LOG = []
    const l = () => console.error(log)
    if (node.location === 'node_modules/typescript') {
      process.on('npm:beforeExit', l)
    }

    process.emit('time', `reifyNode:${node.location}`)
    this.addTracker('reify', node.name, node.location)

    log.push('START', node.location)
    const p = this[_checkEngineAndPlatform](node)
      .then(() => log.push('ABOUT TO EXTRACT', node.location))
      .then(() => this[_extractOrLink](node))
      .then(() => log.push('ABOUT TO EXTRACT', node.location))
      .then(() => this[_warnDeprecated](node))
      .then(() => log.push('ABOUT TO CHECK ENGINE AGAIN', node.location))
      .then(() => this[_recheckEngineAndPlatform](node))
      .then(() => log.push('DONE WITH MAIN STEPS', node.location))

    return this[_handleOptionalFailure](node, p)
      .then(() => {
        log.push('BACK FROM HANDLE OPTIONAL FAILURE', node.location)
        this.finishTracker('reify', node.name, node.location)
        process.emit('timeEnd', `reifyNode:${node.location}`)
        process.removeListener('npm:beforeExit', l)
        return node
      })
  }
  ...
  [_extractOrLink] (node) {
    ...
    const res = node.resolved ? this[_registryResolved](node.resolved)
      ...
      : null
    const log = this._REIFY_LOG
    log.push(['resolved', res])

    // no idea what this thing is.  remove it from the tree.
    if (!res) {
      node.parent = null
      this[_addNodeToTrashList](node)
      log.push(['no resolved, trash it'])
      return
    }
    ...
    return node.isLink
      ...
      : pacote.extract(res, node.path, {
          ...
          log.push(['back from pacote', node.path, res])
          return res
        })
  }
  ...
  [_handleOptionalFailure] (node, p) {
    if (node.optional) {
      console.error('OPTIONAL MIGHT FAIL', node.location)
    }

    return (node.optional ? p.catch(er => {
      console.error('OPTIONAL FAIL', node.location, er)
      const set = optionalSet(node)
      for (node of set) {
        console.error('>> add to trash list', node.location)
        this[_addNodeToTrashList](node)
      }
    }) : p).then(() => node)
  }
  ...
  [_awaitQuickAudit] () {
    process.emit('time', 'auditReport:await')
    return Promise.resolve(this.auditReport).then(rep => {
      process.emit('timeEnd', 'auditReport:await')
      return rep
    })
  }
  ...
  [_unpackNewModules] () {
    this.log.warn('START UNPACK NEW MODULES')
    process.emit('time', 'reify:unpack')
    const unpacks = []

    const upSched = new Set()
    ...
    dfwalk({
      ...
      visit: diff => {
        ...
        if (!doUnpack)
          this.log.warn('-- NO UNPACK', node.location || 'ROOT')

        if (doUnpack) {
          if (upSched.has(node.location)) {
            this.log.warn('dupe unpack, failing', node.location)
            throw new Error('DUPLICATE UNPACK!!?!?')
          }
          upSched.add(node.location)
          unpacks.push(this[_reifyNode](node).then(() => upSched.delete(node.location)))
        }
      },
      getChildren: diff => diff.children,
    })
    process.on('npm:beforeExit', () => {
      console.error('DANGLING UNPACKS?', upSched)
    })

    return promiseAllRejectLate(unpacks)
      .then(() => this.log.warn('FINISHED UNPACKS'))
