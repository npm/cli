...
module.exports = cls => class Auditor extends cls {
  async audit (options = {}) {
    ...
    if (options.package.length > 0) {
      if (options.filterSet !== undefined) {
        const nodes = tree.inventory.query('packageName', options.package)
        options.filterSet.add(...nodes)
      }
      else {
        options.filterSet = options.package.reduce((filterSet, name) => {
            const nodes = tree.inventory.query('packageName', name)
            return filterSet.add(...nodes)
        }, new Set())
      }
    }
