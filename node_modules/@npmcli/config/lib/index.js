...
class Config {
  ...
  async load () {
    ...
    // console.error({
    //   'workspace-root': this[_get]('workspace-root'),
    //   'workspace': this[_get]('workspace'),
  ...
  async loadProjectConfig () {
    ...
    const projectDir = await this.findProjectDir(this.cwd)
    ...
    if (projectFile === this[_get]('userconfig')) {
      ...
      this.data.get('workspace').source = '(same as "user" config, ignored)'
    ...
    if (wsRoot) {
      ...
      this[_set]('workspace-root', wsRoot, 'workspace')
      this[_set]('workspace', [relative(this.localPrefix, projectDir)], 'workspace')
      return
    } else {
      this.localPrefix = projectDir
  ...
  async findProjectDir (start, end = resolve('/')) {
  ...
  async loadLocalPrefix () {
    ...
    this.localPrefix = await this.findProjectDir(this.cwd)
  }
  ...
  async save (where) {
    ...
    const data = conf.data['workspace-root'] ? {
      ...conf.data,
      'workspace-root': relative(dirname(conf.source), conf.data['workspace-root']),
    } : conf.data
