...
class CacacheWriteStream extends Flush {
  ...
  write (chunk, encoding, cb) {
    if (!this.handleContentP) {
      log.warn('FIRST CACHE WRITE', this.opts.url)
    ...
    log.warn('CACHE WRITE RETURN', ret, this.opts.url)
    return ret
  }

  flush (cb) {
    log.warn('CACHE STREAM FLUSH', this.opts.url)
    this.inputStream.end(() => {
      log.warn('CACHE INPUT STREAM END CB', this.opts.url)
      if (!this.handleContentP) {
      ...
      this.handleContentP.then(
        (res) => {
          log.warn('CACHE INPUT STREAM HANDLE THEN', this.opts.url)
...
function handleContent (inputStream, cache, opts) {
  log.warn('HANDLE CONTENT', opts.url)
  return disposer(makeTmp(cache, opts), makeTmpDisposer, (tmp) => {
    log.error('hC CALL PIPE TO TMP', tmp.target, opts.url)
    return pipeToTmp(inputStream, cache, tmp.target, opts)
      .then((res) => {
        log.warn('MOVE TO DESTINATION', opts.url)
