...
class CacacheWriteStream extends Flush {
  ...
  write (chunk, encoding, cb) {
    if (!this.handleContentP) {
      log.warn('FIRST CACHE WRITE', this.opts.metadata.url)
    ...
    const ret = this.inputStream.write(chunk, encoding, cb)
    log.warn('CACHE WRITE RETURN', ret, this.opts.metadata.url, this.name)
    return ret
  }

  end (...args) {
    log.warn('CACHE STREAM .end()', this.opts.metadata.url)
    return super.end(...args)
  }

  flush (cb) {
    log.warn('CACHE STREAM FLUSH', this.opts.metadata.url)
    this.inputStream.end(() => {
      log.warn('CACHE INPUT STREAM END CB', this.opts.metadata.url)
      if (!this.handleContentP) {
      ...
      this.handleContentP.then(
        (res) => {
          log.warn('CACHE INPUT STREAM HANDLE THEN', this.opts.metadata.url)
...
function handleContent (inputStream, cache, opts) {
  log.warn('HANDLE CONTENT', opts.metadata.url)
  return disposer(makeTmp(cache, opts), makeTmpDisposer, (tmp) => {
    log.error('hC CALL PIPE TO TMP', tmp.target, opts.metadata.url)
    return pipeToTmp(inputStream, cache, tmp.target, opts)
      .then((res) => {
        log.warn('MOVE TO DESTINATION', opts.metadata.url)
