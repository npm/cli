...
class CacacheWriteStream extends Flush {
  ...
  write (chunk, encoding, cb) {
    if (!this.handleContentP) {
      if (/8MSzRw==/.test(String(this.opts.integrity)))
        log.warn('FIRST CACHE WRITE', this.opts.integrity)
    ...
    log.warn('CACHE WRITE RETURN', ret)
    return ret
  }

  flush (cb) {
    log.warn('CACHE STREAM FLUSH')
...
function handleContent (inputStream, cache, opts) {
  if (/8MSzRw==/.test(String(opts.integrity)))
    log.warn('HANDLE CONTENT', opts.integrity)
  return disposer(makeTmp(cache, opts), makeTmpDisposer, (tmp) => {
    log.error('hC CALL PIPE TO TMP', tmp.target)
    return pipeToTmp(inputStream, cache, tmp.target, opts)
      .then((res) => {
        if (/8MSzRw==/.test(String(res.integrity)))
          log.warn('MOVE TO DESTINATION', String(res.integrity))
