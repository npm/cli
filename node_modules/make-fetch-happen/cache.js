...
module.exports = class Cache {
  ...
  put (req, response, opts) {
    ...
    if (fitInMemory) {
      ...
      cacheStream.on('drain', () => log.warn('CACHE STREAM DRAIN', req.url, response.status))
      const {pause} = tee
      tee.pause = function () {
        log.warn('TEE PAUSE')
        return pause.call(tee)
      }
      tee.on('resume', () => log.warn('TEE RESUME', req.url, response.status))
      tee.on('end', () => log.warn('TEE END', req.url, response.status))
      tee.on('data', c => log.warn('TEE DATA', req.url, response.status, c.length, tee.flowing || [...tee.pipes].map(p => [p.dest.buffer.length, p.dest.flowing])))
      cacheStream.on('end', () => log.warn('CACHE STREAM END', req.url, response.status))
