...
class FetcherBase {
    ...
    const fromResolved = er => {
      if (er) {
        ...
        this.log.silly('tarball', `no local data for ${
          this.spec
        }. Extracting by manifest.`, er.stack)
      }
  ...
  [_extract] (dest, tarball, uid, gid) {
    if (/typescript/.test(dest))
      this.log.silly('tar', 'EXTRACT', { dest, resolved: this.resolved, stack: new Error().stack })

    const extractor = tar.x(this[_tarxOptions]({ cwd: dest, uid, gid }))
    const p = new Promise((resolve, reject) => {
      extractor.on('end', () => {
        if (/typescript/.test(dest))
          this.log.silly('tar', 'done EXTRACT', { resolved: this.resolved, dest })
        resolve({
          resolved: this.resolved,
          integrity: this.integrity && String(this.integrity),
          from: this.from,
      ...
      extractor.on('error', er => {
        if (/typescript/.test(dest)) {
          this.log.warn('UNPACK EXTRACTOR ERROR', er)
        }
      ...
      tarball.on('error', er => {
        if (/typescript/.test(dest)) {
          this.log.warn('UNPACK TARBALL ERROR', er)
        }
        reject(er)
      })
      if (/typescript/.test(dest)) {
        extractor.on('drain', () => {
          this.log.warn('EXTRACTOR DRAIN', { dest }, extractor.pending)
        })

        tarball.on('pause', () => {
          this.log.warn('TARBALL PAUSE', { dest })
        })
        tarball.on('resume', () => {
          this.log.warn('TARBALL RESUME', { dest })
        })
        extractor.on('pause', () => {
          this.log.warn('EXTRACTOR PAUSE', { dest })
        })
        extractor.on('resume', () => {
          this.log.warn('EXTRACTOR RESUME', { dest })
        })
      }
      ...
      //  tarball.on('pause', () => {
      //    t = setTimeout(() => {
      //      console.error('PAUSED AND DID NOT RESUME WITHIN 100MS', { dest })
      //    }, 100)
      //    t.unref()
      //  })
      //  tarball.on('resume', () => {
      ...
      //    if (t) {
      //      console.error('PAUSED AND DID NOT RESUME', { dest })
      //    }
      //  })
      //}
    })
