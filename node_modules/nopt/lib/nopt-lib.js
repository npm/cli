...
const getType = (k, { types, fallbackTypes }) => {
  let hasType = hasOwn(types, k)
  let type = types[k]
  if (!hasType && typeof fallbackTypes === 'function') {
    const matchedType = fallbackTypes(k)
...
function nopt (args, { types, fallbackTypes, shorthands, typeDefs, invalidHandler, typeDefault }) {
  ...
  parse(args, data, argv.remain, { typeDefs, types, fallbackTypes, shorthands })

  // now data is full
  clean(data, { types, fallbackTypes, typeDefs, invalidHandler, typeDefault })
  data.argv = argv
