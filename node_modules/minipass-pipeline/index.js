...
class Pipeline extends Minipass {
  ...
  [_onData] (stream, chunk) {
    if (stream === this[_tail]) {
      console.error('PIPELINE TAIL DATA', this.name, stream.name)
      super.write(chunk)
    }
  }
  [_onEnd] (stream) {
    if (stream === this[_tail]) {
      console.error('PIPELINE TAIL END', this.name, stream.name)
      super.end()
    }
  }
  pause () {
    super.pause()
    console.error('PIPELINE PAUSE', this.name, this[_tail] && this[_tail].name)
  ...
  emit (ev, ...args) {
    console.error('PIPELINE RESUME', this.name, this[_tail] && this[_tail].name)
