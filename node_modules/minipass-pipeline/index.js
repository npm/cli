...
class Pipeline extends Minipass {
  ...
  get flowing () {
    return super.flowing || this.buffer.length === 0 && this[_tail] && this[_tail].flowing
  }
  write (chunk, enc, cb) {
    // 
    const ret = this[_head].write(chunk, enc, cb)
    if (ret && !this.flowing && this.buffer.length === 0) {
      console.error('PIPELINE NOT FLOWING BUT HEAD RETURNS TRUE??', this.name, this.buffer.length, this.pipes.length, preflowing)
    }
    return ret && this.flowing //(this.flowing || this.buffer.length === 0 && this[_tail] && this[_tail].flowing)
