...
the working directory is within a workspce and `workspaces` is not `true`.
In that case, no commit is made.  This behavior is controlled by
`git-tag-version` (see below), and can be disabled on the command line by
running `npm --no-git-tag-version version`.  It will fail if the working
directory is not clean, unless the `-f` or `--force` flag is set.
