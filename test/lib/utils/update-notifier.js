...
t.test('notifies by default', async t => {
...
t.test('notification situations', async t => {
  const cases = {
    [HAVE_BETA]: [`^{V}`, `^{V}`],
    [NEXT_PATCH]: [`latest`, `^{V}`, 'latest', `^{V}`],
    [NEXT_MINOR]: [`latest`, `^{V}`, 'latest', `^{V}`],
    [CURRENT_PATCH]: ['latest', 'latest'],
    [CURRENT_MINOR]: ['latest', 'latest'],
    [CURRENT_MAJOR]: ['latest', 'latest'],
  }
