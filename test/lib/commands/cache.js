...
const setupCacacheFixture = () => {
  ...
  cacacheContent[
    [
      cacacheEntries['make-fetch-happen:request-cache:https://registry.npmjs.org/corrupted']
        .integrity,
    ]
  ].data = Buffer.from('<>>>}"')
  ...
  cacacheContent[
    [
      cacacheEntries['make-fetch-happen:request-cache:https://registry.npmjs.org/missing-dist']
        .integrity,
    ]
  ...
  cacacheContent[
    [
      cacacheEntries['make-fetch-happen:request-cache:https://registry.npmjs.org/missing-version']
        .integrity,
    ]
...
t.test('cache no args', async t => {
  await t.rejects(cache.exec([]), 'usage instructions', 'should throw usage instructions')
})

t.test('cache clean', async t => {
  await t.rejects(cache.exec(['clean']), 'the npm cache self-heals', 'should throw warning')
})
...
t.test('cache add no arg', async t => {
  ...
  await t.rejects(cache.exec(['add']), { code: 'EUSAGE' }, 'throws usage error')
  t.strictSame(logOutput, [['silly', 'cache add', 'args', []]], 'logs correctly')
})
...
t.test('cache ls tagged', async t => {
  ...
  await t.rejects(cache.exec(['ls', 'webpack@latest']), 'tagged package', 'should throw warning')
})
...
t.test('cache rm unfound', async t => {
  ...
  t.strictSame(logOutput, [['warn', 'Not Found: made-up-key']], 'logs correctly')
})
