...
t.test('trust-gitlab with all options provided', async t => {
...
t.test('trust-gitlab with invalid project format', async t => {
  const { npm } = await loadMockNpm(t, {
    prefixDir: {
      'package.json': JSON.stringify({
        name: packageName,
...
t.test('trust-gitlab with file as path', async t => {
  const { npm } = await loadMockNpm(t, {
    prefixDir: {
      'package.json': JSON.stringify({
        name: packageName,
...
t.test('trust-gitlab without environment', async t => {
  const { npm } = await loadMockNpm(t, {
    prefixDir: {
      'package.json': JSON.stringify({
        name: packageName,
...
t.test('bodyToOptions with all fields', t => {
  const TrustGitLab = require('../../../lib/subcommands/trust-gitlab.js')
