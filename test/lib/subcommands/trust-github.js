...
t.test('trust-github with all options provided', async t => {
...
t.test('trust-github with invalid repository format', async t => {
  const { npm } = await loadMockNpm(t, {
    prefixDir: {
      'package.json': JSON.stringify({
        name: packageName,
...
t.test('trust-github with file as path', async t => {
  const { npm } = await loadMockNpm(t, {
    prefixDir: {
      'package.json': JSON.stringify({
        name: packageName,
...
t.test('trust-github without environment', async t => {
  const { npm } = await loadMockNpm(t, {
    prefixDir: {
      'package.json': JSON.stringify({
        name: packageName,
...
t.test('bodyToOptions with all fields', t => {
  const TrustGitHub = require('../../../lib/subcommands/trust-github.js')
