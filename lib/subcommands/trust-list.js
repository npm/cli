...
const TrustGithub = require('./trust-github.js')
const TrustGitlab = require('./trust-gitlab.js')
const TrustCommand = require('../trust-cmd.js')
const globalDefinitions = require('@npmcli/config/lib/definitions/definitions.js')

// Convert trust config body to options object
...
class TrustList extends TrustCommand {
  ...
  async exec (args) {
    ...
    const res = await otplease(this.npm, this.npm.flatOptions, opts => npmFetch.json(uri, {
      ...opts,
      method: 'GET',
    }))

    if (!res || res.length === 0) {
      this.warn`No trust configurations found for package (${packageName})`
      return
    }
    this.notice`Trust configurations for package (${packageName}):`
    for (const config of res) {
      const values = bodyToOptions(config)
