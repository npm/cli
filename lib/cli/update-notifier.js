...
const updateCheck = async (npm, spec, version, current) => {
  ...
  const message = [
    '',
    `New ${color.stderr(typec, type)} version of npm available! ${color.stderr(typec, String(current))} -> ${color.stderr('blue', latest)}`,
    `Changelog: ${color.stderr('blue', `https://github.com/npm/cli/releases/tag/v${latest}`)}`,
    `To update run: ${color.stderr('underline', `npm install -g npm@${latest}`)}`,
    '',
  ].join('\n')
