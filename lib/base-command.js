...
class BaseCommand {
  ...
  static get usage () {
    // XXX: this could use wrap util which reads from stdout.columns
    const wrapWidth = 80
    ...
    if (params) {
      ...
      for (const param of params) {
        const paramUsage = `[${definitions[param].describeUsage()}]`
  ...
  constructor (npm) {
    ...
    // add getters for some statics to make reading them easier from an
    // instantiated class
    for (const key of ['name', 'description', 'params', 'usage']) {
