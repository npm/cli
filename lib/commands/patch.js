...
class Patch extends BaseCommand {
  ...
  async commit (args) {
    ...
    // only treat the marker as an update finalize when it renames a real, same-package selector,
    // so a stray or forged marker file can never hijack a normal commit or drop an unrelated entry
    const updateFinalize = !!marker?.removeKey &&
      marker.removeKey !== newKey &&
    ...
    try {
      if (rebase.conflicted) {
        ...
        try {
          ...
          // an exact selector is renamed, so tell the finalizing commit to drop the old key
          if (entry.spec && semver.valid(entry.spec)) {
  ...
  #resolveNewVersion (entry, installedVersions) {
    ...
    const stale = patchErr(
      `lockfile is stale or missing for "${entry.name}" — pass --to <new-version> or run "npm install" first`,
      'EPATCHSTALE'
    )
