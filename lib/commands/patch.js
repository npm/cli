...
class Patch extends BaseCommand {
  ...
  async #resolveTarget (spec) {
    ...
    // a version is patchable only when every consumer-facing node resolves from the registry.
    // store entries under .store are content artifacts with no edges, so they are excluded from the check.
    // an explicit version with nothing installed is allowed; the spec was already verified as a registry spec above.
    const ensureRegistry = version => {
      const nodes = (installed.get(version) || [])
        .filter(n => !n.location?.includes('node_modules/.store/'))
      if (nodes.length && !nodes.every(n => n.isRegistryDependency)) {
  ...
  async commit (args) {
    ...
    if (!relPatch || relPatch.startsWith('..') || isAbsolute(relPatch)) {
      throw Object.assign(
