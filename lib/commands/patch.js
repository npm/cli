...
class Patch extends BaseCommand {
  ...
  async #resolveTarget (spec) {
    ...
    // a version is patchable when any of its nodes resolves from the registry.
    // under install-strategy=linked a registry dep is a symlink to a store entry, so checking for a registry resolution avoids rejecting it.
    ...
    const ensureRegistry = version => {
      ...
      if (nodes.length && !nodes.some(n => n.isRegistryDependency)) {
