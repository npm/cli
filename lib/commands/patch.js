...
class Patch extends BaseCommand {
  ...
  async #resolveTarget (spec) {
    ...
    // a version cannot be patched if any of its installed nodes is non-registry
    const ensureRegistry = version => {
      const nodes = installed.get(version) || []
      if (nodes.some(n => n.isLink || !n.isRegistryDependency)) {
  ...
  async ls () {
    ...
    // names with overlapping ranges that install would reject as ambiguous
