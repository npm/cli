...
class Patch extends BaseCommand {
  ...
  // Record that an edit dir holds an npm: alias of source.
  async #writeAliasMarker (editDir, name, source) {
    if (name !== source) {
      await writeFile(join(editDir, ALIAS_MARKER), JSON.stringify({ name, source }) + '\n')
    }
  }
  ...
  async #makeEditDir (name, version) {
    ...
    // clear a stale alias marker before extracting, so a file the package ships is never touched
    await rm(join(editDir, ALIAS_MARKER), { force: true })
    return editDir
  }
