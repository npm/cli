...
class Patch extends BaseCommand {
  ...
  async #resolveTarget (spec) {
    ...
    // a node that is not a registry dependency cannot be patched
    const ensureRegistry = node => {
      if (node && (node.isLink || !node.isRegistryDependency)) {
        throw this.#nonRegistryError(`${name}@${node.version}`)
      }
    }
    ...
    if (parsed.rawSpec && parsed.rawSpec !== '*' && parsed.rawSpec !== 'latest') {
      ...
      if (exact) {
        ensureRegistry(installed.get(exact))
        return { name, version: exact }
      }
      const matches = [...installed.entries()]
        .filter(([v]) => semver.satisfies(v, parsed.fetchSpec))
      if (matches.length > 1) {
        throw this.#ambiguousError(name, matches)
      }
      if (matches.length === 1) {
        ensureRegistry(matches[0][1])
        return { name, version: matches[0][0] }
      }
    ...
    if (installed.size > 1) {
      throw this.#ambiguousError(name, [...installed.entries()])
    }
    const [version, node] = [...installed.entries()][0]
    ensureRegistry(node)
    return { name, version }
  }
  ...
  #ambiguousError (name, entries) {
    const lines = entries.map(([version, node]) => {
    ...
    return Object.assign(
      new Error(`Multiple versions of "${name}" are installed:\n${lines.join('\n')}\n` +
        `Re-run with an exact selector, e.g. "npm patch add ${selectorKey(name, entries[0][0])}".`),
  ...
  async ls () {
    ...
    for (const node of tree.inventory.values()) {
      ...
      try {
        ...
        // ambiguous overlapping ranges surface at install time, skip here
        continue
      }
      if (winner) {
