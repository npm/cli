...
class Diff extends BaseCommand {
  ...
  async retrieveSpecs ([a, b]) {
    ...
    if (spec.registry) {
      ...
      if (spec.rawSpec) {
        ...
        bSpec = pickManifest(packument, bTargetVersion, { ...this.npm.flatOptions }).version
      }
  ...
  async findVersionsByPackageName (specs) {
    ...
    return specs.map(i => {
      ...
      const node =
        actualTree && actualTree.inventory.query('name', spec.name).values().next().value

      const res =
        !node || !node.package || !node.package.version ? spec.fetchSpec : `file:${node.realpath}`
