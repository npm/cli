  ...
  async exec (args) {
    ...
    const result = await breadth({
      ...
      visit (node) {
        ...
        const item = json
          ...
          : parseable
            ? null
            : getHumanOutputItem(node, { args, global, long })
    ...
    if (json) {
      ...
      output.standard(parseable
        ...
        : humanOutput({ result, seenItems, unicode })
      )
    }
...
const getHumanOutputItem = (node, { args, global, long }) => {
  const { pkgid, path } = node
  const workspacePkgId = color.stdout('blueBright', pkgid)
  ...
  const missingColor = isOptional(node)
    ? (t) => color.stdout('yellow', t)
    : (t) => color.stdout('red', t)
  ...
  const label =
    ...
    `${highlightDepName ? color.stdout('yellow', printable) : printable}` +
    (
      node[_dedupe]
        ? ' ' + color.stdout('dim', 'deduped')
        : ''
    ) +
    (
      invalid
        ? ' ' + color.stdout('red', invalid)
        : ''
    ) +
    (
      isExtraneous(node, { global })
        ? ' ' + color.stdout('red', 'extraneous')
        : ''
    ) +
    (
      node.overridden
        ? ' ' + color.stdout('dim', 'overridden')
        : ''
    ) +
    (
      node.patched
        ? ' ' + color.stdout('cyan', `[patched: ${node.patched.path}]`)
        : ''
    ) +
    (
      formatExtensionApplied(node.packageExtensionsApplied)
        ? ' ' + color.stdout('dim', `packageExtensions: ${formatExtensionApplied(node.packageExtensionsApplied)}`)
        : ''
    ) +
    (
      formatExtensionApplied(node.npmExtensionApplied)
        ? ' ' + color.stdout('dim', `.npm-extension: ${formatExtensionApplied(node.npmExtensionApplied)}`)
        : ''
    ) +
...
const humanOutput = ({ result, seenItems, unicode }) => {
  ...
  return color.stdout('reset', archyOutput)
}
