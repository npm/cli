...
const filterByEdgesTypes = ({ link, omit, currentDepth }) => (edge) => {
  for (const omitType of omit) {
    if (edge[omitType] && (currentDepth === 0 || omitType === 'optional')) {
      return false
    }
  }
  return currentDepth === 0 && link ? edge.to && edge.to.isLink : true
}
