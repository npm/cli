...
class CI extends ArboristWorkspaceCmd {
  ...
  async exec () {
    ...
    if (this.npm.flatOptions.before) {
      // `npm ci` installs the lockfile verbatim and never re-resolves, so resolver-time filters like `--before` and `--min-release-age` have no effect here.
      // Warn so users don't think the cooldown is being enforced when it isn't, and point them at `npm audit min-release-age` for lockfile validation.
      log.warn(
        'ci',
        '`--before` and `--min-release-age` have no effect on `npm ci`: it installs the lockfile verbatim and does not re-resolve dependencies. Run `npm audit min-release-age` to validate an existing lockfile against the configured cutoff.'
      )
    }
