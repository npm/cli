...
const AuditMinReleaseAge = require('../utils/audit-min-release-age.js')
...
class Audit extends ArboristWorkspaceCmd {
  ...
  static usage = ['[fix|signatures|min-release-age]']

  static async completion (opts) {
    const argv = opts.conf.argv.remain

    if (argv.length === 2) {
      return ['fix', 'signatures', 'min-release-age']
    }
  ...
  async exec (args) {
    ...
    } else if (args[0] === 'min-release-age') {
      await this.auditMinReleaseAge()
    } else {
  ...
  async auditMinReleaseAge () {
    if (this.npm.global) {
      throw Object.assign(
        new Error('`npm audit min-release-age` does not support global packages'), {
          code: 'EAUDITGLOBAL',
        }
      )
    }

    log.verbose('audit', 'loading dependencies from lockfile')
    const Arborist = require('@npmcli/arborist')
    const opts = {
      ...this.npm.flatOptions,
      path: this.npm.prefix,
    ...
    try {
      ...
      throw this.usageError(
        '`npm audit min-release-age` requires an existing package-lock.json ' +
        'with lockfileVersion >= 1. Run `npm install` first to generate one.'
      )
    }
    ...
    const audit = new AuditMinReleaseAge(tree, filterSet, this.npm, { ...opts })
    await audit.run()
  }
}
