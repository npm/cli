...
class Link extends ArboristWorkspaceCmd {
  ...
  async linkInstall (args) {
    ...
    const add = names.map(l => `file:${resolve(globalTop, 'node_modules', l)}`)
    const paths = this.workspacePaths?.length ? this.workspacePaths : [this.npm.prefix]

    // Workspace-targeted npm link should create the symlink inside each selected
    // workspace rather than reifying at the project root and relying on hoisting.
