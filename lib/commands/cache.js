...
const searchCachePackage = async (path, spec, cacheKeys) => {
  ...
  const searchMFH = new RegExp(
    /* eslint-disable-next-line max-len */
    `^make-fetch-happen:request-cache:.*(?<!/[@a-zA-Z]+)/${parsed.name}/-/(${parsed.name}[^/]+.tgz)$`
  )
...
class Cache extends BaseCommand {
  ...
  async verify () {
    ...
    const prefix =
      cache.indexOf(process.env.HOME) === 0 ? `~${cache.substr(process.env.HOME.length)}` : cache
