...
If you're sure you want to delete the entire cache, rerun this command with --force.`)
  ...
  async npxInfo (keys) {
    ...
    for (const key in cache) {
          ...
          if (pkg._npx) {
            ...
            for (const p of pkg._npx.packages) {
              ...
              if (parsed.type === 'directory') {
                ...
                results.push(`- ${color.stdout('cyan', p)}`)
              } else {
                results.push(`- ${color.stdout('cyan', p)} (${color.stdout('blue', tree.children.get(parsed.name).pkgid)})`)
              }
            }
          } else {
            ...
            for (const dep in pkg.dependencies) {
              ...
              if (child.isLink) {
                results.push(`- ${color.stdout('cyan', child.realpath)}`)
              } else {
                results.push(`- ${color.stdout('cyan', child.pkgid)}`)
              }
            }
          }
        }
      ...
      const v = valid ? color.stdout('green', 'valid') : color.stdout('red', 'invalid')
      output.standard(`${v} npx cache entry with key ${color.stdout('blue', hash)}`)
      output.standard(`location: ${color.stdout('blue', path)}`)
      if (valid) {
