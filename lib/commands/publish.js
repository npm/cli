...
const replaceInfo = require('@npmcli/config/lib/replace-info.js')
...
class Publish extends BaseCommand {
  ...
  async exec (args) {
    ...
    const creds = this.npm.config.credentials.getByURI(registry)
  ...
  async getManifest (spec, opts) {
    ...
    if (manifest.publishConfig) {
      ...
      this.npm.config.load('publishConfig', manifest.publishConfig)
    }
    return manifest
  }
}
