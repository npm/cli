...
class Token extends BaseCommand {
  ...
  async list () {
    ...
    for (const token of tokens) {
      ...
      output.standard(`${color.stdout('blue', 'Token')} ${token.token}… with id ${color.stdout('cyan', token.id)} name ${color.stdout('magenta', token.name)} created ${created}`)
      if (token.cidr_whitelist) {
        output.standard(`with IP whitelist: ${color.stdout('green', token.cidr_whitelist.join(','))}`)
      }
      output.standard()
    }
  }
  ...
  async create () {
    ...
    if (json) {
      ...
      if (result.cidr_whitelist?.length) {
        output.standard(`with IP whitelist: ${color.stdout('green', result.cidr_whitelist.join(','))}`)
      }
      if (result.expires) {
