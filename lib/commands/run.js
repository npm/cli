...
class RunScript extends BaseCommand {
  ...
  async #list (path, { workspace } = {}) {
    ...
    const pkgId = `in ${color.stdout('cyan', _id || name)}`
    const title = (t) => color.stdout('reset', color.stdout('bold', t))

    if (cmds.length) {
      output.standard(`${title('Lifecycle scripts')} included ${pkgId}:`)
      for (const [k, v] of cmds) {
        output.standard(`  ${k}`)
        output.standard(`    ${color.stdout('dim', v)}`)
      }
    }

    if (runScripts.length) {
      const via = `via \`${color.stdout('blue', 'npm run')}\`:`
      if (!cmds.length) {
