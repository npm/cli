...
class Outdated extends ArboristWorkspaceCmd {
  ...
  #pretty (list) {
    ...
    return table([
      ...
      ].map(h => color.stdout(['bold', 'underline'], h)),
      ...list.map((d) => [
        d.current === d.wanted ? color.stdout('yellow', d.name) : color.stdout('red', d.name),
        d.current ?? 'MISSING',
        color.stdout('cyan', d.wanted),
        color.stdout('blue', d.latest),
        d.location ?? '-',
        d.workspaceDependent ? color.stdout('blue', d.workspaceDependent) : d.dependent,
        ...long ? [d.type, color.stdout('blue', d.homepage ?? ''), d.dependedByLocation] : [],
      ]),
    ], {
      align: ['l', 'r', 'r', 'r', 'l'],
