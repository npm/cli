...
class View extends BaseCommand {
  ...
  #prettyView (packu, manifest) {
    ...
    res.push('')
    res.push([
      color.stdout(['underline', 'cyan'], `${manifest.name}@${manifest.version}`),
      license.toLowerCase().trim() === 'proprietary'
        ? color.stdout('red', license)
        : color.stdout('green', license),
      `deps: ${deps.length ? color.stdout('cyan', deps.length) : color.stdout('cyan', 'none')}`,
      `versions: ${color.stdout('cyan', packu.versions.length + '')}`,
    ].join(' | '))

    manifest.description && res.push(manifest.description)
    if (site) {
      res.push(color.stdout('blue', site))
    }

    manifest.deprecated && res.push(
      `\n${color.stdout('redBright', 'DEPRECATED')}${unicode ? ' ⚠️ ' : '!!'} - ${manifest.deprecated}`
    )

    if (packu.keywords?.length) {
      res.push(`\nkeywords: ${
        packu.keywords.map(k => color.stdout('cyan', k)).join(', ')
      }`)
    }

    if (bins.length) {
      res.push(`\nbin: ${color.stdout('cyan', bins.join(', '))}`)
    }

    res.push('\ndist')
    res.push(`.tarball: ${color.stdout('blue', manifest.dist.tarball)}`)
    res.push(`.shasum: ${color.stdout('green', manifest.dist.shasum)}`)
    if (manifest.dist.integrity) {
      res.push(`.integrity: ${color.stdout('green', manifest.dist.integrity)}`)
    }
    if (manifest.dist.unpackedSize) {
      res.push(`.unpackedSize: ${color.stdout('blue', formatBytes(manifest.dist.unpackedSize, true))}`)
    }
    ...
    if (deps.length) {
      ...
      if (deps.length > maxDeps) {
        res.push(color.stdout('dim', `(...and ${deps.length - maxDeps} more.)`))
      }
    }
    ...
    if (packu.maintainers?.length) {
      ...
      packu.maintainers.forEach(u =>
        res.push(`- ${unparsePerson({
          name: color.stdout('blue', u.name),
          email: color.stdout('dim', u.email) })}`)
      )
    }
    ...
    if (distTags.length > maxTags) {
      res.push(color.stdout('dim', `(...and ${distTags.length - maxTags} more.)`))
    }

    const publisher = manifest._npmUser && unparsePerson({
      name: color.stdout('blue', manifest._npmUser.name),
      email: color.stdout('dim', manifest._npmUser.email),
    })
    if (publisher || packu.time) {
      let publishInfo = 'published'
      if (packu.time?.[manifest.version]) {
        publishInfo += ` ${color.stdout('cyan', relativeDate(packu.time[manifest.version]))}`
      }
      if (publisher) {
