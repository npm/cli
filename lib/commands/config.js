const { keys: configKeys, nerfDarts, definitions } = require('@npmcli/config')
...
class Config extends BaseCommand {
  ...
  async set (args) {
    ...
    for (const [key, val] of Object.entries(keyValues(args))) {
      ...
      const def = definitions[baseKey]
      if (!def && !nerfDarts.includes(baseKey)) {
        throw new Error(`\`${baseKey}\` is not a valid npm option`)
      }
      const deprecated = def[baseKey]?.deprecated
      if (deprecated) {
        throw new Error(
  ...
  async edit () {
    ...
    const entries = Object.entries(this.npm.config.defaults)
