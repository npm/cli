...
const parseEditor = (editor) => {
  ...
  // Windows editor paths commonly contain spaces and are often stored without
  // quotes (for example, C:\\Program Files\\Editor\\editor.exe).
  if (/^[a-z]:[\\/]/i.test(parsed[0] || '')) {
    const executable = parsed.findIndex((arg, index) =>
      index > 0 && /\.(?:exe|cmd|bat|com)$/i.test(arg))
    if (executable > 0) {
      parsed.splice(0, executable + 1, parsed.slice(0, executable + 1).join(' '))
    }
  }
