...
class Team extends BaseCommand {
  ...
  async listUsers (entity, opts) {
    ...
    if (opts.json) {
      ...
      output.standard(`${chalk.blue(`@${entity}`)} has ${users.length} user${users.length === 1 ? '' : 's'}${users.length ? ':' : ''}`)
      output.standard(users.join('\n'))
    }
  }
  ...
  async listTeams (entity, opts) {
    ...
    if (opts.json) {
      ...
      /* eslint-disable-next-line max-len */
      output.standard(`${chalk.blue(`@${entity}`)} has ${teams.length} team${teams.length === 1 ? '' : 's'}${teams.length ? ':' : ''}`)
      output.standard(teams.map(t => `@${t}`).join('\n'))
    }
  }
}
module.exports = Team
