...
class BaseCommand {
  ...
  static getUsage (parentName = null, includeDescriptions = true) {
    ...
    if (this.subcommands) {
      ...
      fullUsage.push('')
      fullUsage.push(`Run "npm ${fullCommandName} <subcommand> --help" for more info on a subcommand.`)
    }
    ...
    fullUsage.push('')
    const helpCommandName = parentName ? parentName.split(' ')[0] : name
    fullUsage.push(`Run "npm help ${helpCommandName}" for more info`)
  ...
  get usage () {
    const parentName = this.commandPath?.length > 1 ? this.commandPath.slice(0, -1).join(' ') : null
    return this.constructor.getUsage(parentName)
  }
