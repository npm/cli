...
const { getTypesFromDefinitions } = require('@npmcli/config')
...
class BaseCommand {
  ...
  flags () {
    ...
    const { types, shorthands, defaults } = getTypesFromDefinitions(commandDefinitions)
    const parsed = nopt(types, shorthands, this.config.argv)
    delete parsed.argv

    // Only include keys that are in types
    const filtered = {}
