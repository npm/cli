...
class BaseCommand {
  ...
  checkDevEngines () {
    // Eventually the package.json file at localPrefix could be attached to the npm object itself, we read from it a few times (but not in any hot paths).
    const { devEngines } = await require('@npmcli/package-json')
      .normalize(this.localPrefix)
      .then(p => p.content)
      .catch(() => {})
    if (!localPkg.devEngines) {
      return
    }
    const { checkDevEngines } = require('npm-install-checks')
    // get current environment
    // call checkDevEngines
    // throw if unmet constraints
  }
