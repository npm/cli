...
class LogFiles {
  ...
  load ({ path, logsMax = Infinity } = {}) {
    ...
    if (!this.#logStream) {
      log.verbose('logfile', 'no logfile, logstream already closed')
      return
    }

    log.verbose('logfile', `dir:${this.#path}`)
    ...
    if (this.#logsMax > 0) {
      const initialStream = this.#openLogFile()
      if (initialStream) {
        log.verbose('logfile', initialStream.path)
        this.#logStream = this.#logStream.pipe(initialStream)
      }
    } else {
      log.verbose('logfile', `no logfile, logs-max:${logsMax}`)
    }
  ...
  async #cleanLogs () {
    ...
    try {
      ...
      log.silly('logfile',
        `start cleaning logs, files:${files.length} max:${this.#logsMax} removing:${toDelete}`)
