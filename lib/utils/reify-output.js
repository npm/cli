    ...
    depth({
      ...
      visit: d => {
        switch (d.action) {
          case 'REMOVE':
            if (showDiff) {
              output.standard(`${color.stdout('blue', 'remove')} ${d.actual.name} ${d.actual.package.version}`)
            }
            summary.removed++
            summary.remove.push({
              name: d.actual.name,
          ...
          case 'ADD':
            if (showDiff) {
              output.standard(`${color.stdout('green', 'add')} ${d.ideal.name} ${d.ideal.package.version}`)
            }
          ...
          case 'CHANGE':
            if (showDiff) {
              output.standard(`${color.stdout('cyan', 'change')} ${d.actual.name} ${d.actual.package.version} => ${d.ideal.package.version}`)
            }
            summary.changed++
            summary.change.push({
              from: {
