...
const isGlobalNpmUpdate = npm => npm.flatOptions.global &&
  ['install', 'update'].includes(npm.command) &&
  npm.argv.some(arg => /^npm(@|$)/.test(arg))

const skipCheck = (npm) => {
  // never check for updates in CI, when updating npm already, or opted out
  if (!npm.config.get('update-notifier') || isGlobalNpmUpdate(npm) || ciInfo.isCI) {
    return true
  }
}
...
const FILE_NAME = '_update-notifier-last-checked'

// don't put it in the _cacache folder, just in npm's cache
const lastCheckedFile = npm => resolve(npm.flatOptions.cache, '..', FILE_NAME)
...
const updateNotifier = async (npm, spec = 'latest') => {
  ...
  const type = update.major !== current.major ? 'major'
    ...
    : 'prerelease'
  const typeColor = type === 'major' ? 'red' : type === 'minor' ? 'yellow' : 'green'
  const cmd = `npm install -g npm@${latest}`
  const changelog = `https://github.com/npm/cli/releases/tag/v${latest}`

  const { chalk } = npm
  return `\nNew ${chalk[typeColor](type)} version of npm available! ` +
    `${chalk.red(current)} -> ${chalk.green(latest)}\n` +
