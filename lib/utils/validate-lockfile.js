...
function validateLockfile (virtualTree, idealTree) {
  ...
  for (const [key, entry] of idealTree.entries()) {
    ...
    // a patch whose on-disk hash diverges from the lockfile is out of sync
    const lockPatch = lock.patched?.integrity || null
    const entryPatch = entry.patched?.integrity || null
    if (lockPatch !== entryPatch) {
      errors.push(`Invalid: patch for ${entry.name}@${entry.version} does not ` +
      `match the integrity recorded in the lock file`)
    }
  }
  return errors
}
