...
// Parse a raw `allow-scripts` config value (string or array) into a flat
// array of trimmed package names. Mirrors the flatten function in
// workspaces/config/lib/definitions/definitions.js.
const parseConfigValue = (raw) => {
  const parts = []
  /* istanbul ignore next: nopt always returns arrays or undefined here */
  const entries = Array.isArray(raw) ? raw : (typeof raw === 'string' ? [raw] : [])
  for (const entry of entries) {
    /* istanbul ignore if: nopt produces string entries for [String,Array] types */
    if (typeof entry !== 'string') {
      continue
    }
    for (const part of entry.split(',')) {
      const trimmed = part.trim()
      /* istanbul ignore else: split on ',' produces non-empty strings after trim except for edge cases tested via flatten */
      if (trimmed) {
        parts.push(trimmed)
      }
    }
...
const policyFromSources = (npm, sources) => {
  for (const where of sources) {
    ...
    const names = parseConfigValue(value)
    /* istanbul ignore else: parseConfigValue returns non-empty when value is set */
    if (names.length) {
...
const resolveAllowScripts = async (npm, { skipProjectConfig = false } = {}) => {
  ...
  if (cli) {
    if (pkg) {
      log.warn(
        'allow-scripts',
        'package.json#allowScripts is being ignored because --allow-scripts was passed on the command line'
      )
    }
    if (rc) {
      log.warn(
