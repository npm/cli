...
class AllowScriptsCmd extends BaseCommand {
  ...
  findNodesForArgs (args, arb) {
    ...
    // as --all. Links are excluded for the same reason the policy gate skips
    // them: identity lives on the target, while a link only knows the relative
    // path it points at, so under install-strategy=linked a package would
    // otherwise get one `file:` key per symlink depth. Args that match nothing
