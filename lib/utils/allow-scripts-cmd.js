...
// Trusted display identity for `npm approve-scripts --pending` output.
// Same idea as the matcher: prefer the URL-derived name. For DISPLAY
// purposes only (not policy matching), version falls back to node.version
// when the URL doesn't carry one.
...
class AllowScriptsCmd extends BaseCommand {
  static params = ['all', 'pending', 'pin', 'json']
  ...
  async exec (args) {
    ...
    const pending = !!this.npm.config.get('pending')
    const all = !!this.npm.config.get('all')

    if (pending && (args.length > 0 || all)) {
      throw this.usageError(
        '`--pending` cannot be combined with positional arguments or `--all`.'
      )
    }
    ...
    if (this.verb === 'deny' && pending) {
      throw this.usageError('`npm deny-scripts --pending` is not supported.')
    }
  ...
  async writePolicyChanges (groups) {
    const pin = this.npm.config.get('pin') !== false
