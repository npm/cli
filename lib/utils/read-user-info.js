...
// npm's username rules only apply to the npm registry, other registries may
// accept different formats such as email addresses
function readUsername (msg = usernamePrompt, username, isRetry, { validate = true } = {}) {
  if (isRetry && username) {
    const error = validate && userValidate.username(username)
    if (error) {
  ...
  return read({ prompt: msg, default: username || '' })
    .then((rUsername) => readUsername(msg, rUsername, true, { validate }))
}
