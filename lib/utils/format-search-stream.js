...
class TextOutputStream extends Minipass {
  ...
  write (data) {
    ...
    if (this.#args.includes(pkg.name)) {
      ...
      name = this.#highlightName(pkg.name)
    }
  ...
  #highlightName (name) {
    ...
    for (const arg of this.#args) {
      let i = lower.indexOf(arg)
      while (i > -1) {
        for (let j = i; j < i + arg.length; j++) {
          matched[j] = true
        }
        i = lower.indexOf(arg, i + arg.length)
      }
    }
    ...
    while (start < name.length) {
      const isMatch = matched[start]
      let end = start
      while (end < name.length && matched[end] === isMatch) {
        end++
      }
      output += color.stdout(isMatch ? 'cyan' : 'blue', name.slice(start, end))
      start = end
    }
    return output
  }
