...
class VerifySignatures {
  ...
  async run () {
    ...
    const verifiedBold = color.stdout('bold', 'verified')
    ...
    if (missing.length) {
      const missingClr = color.stdout('redBright', 'missing')
      if (missing.length === 1) {
      ...
      output.standard()
      missing.map(m =>
        output.standard(`${color.stdout('red', `${m.name}@${m.version}`)} (${m.registry})`)
      )
    }
    ...
    if (invalid.length) {
      ...
      const invalidClr = color.stdout('redBright', 'invalid')
      ...
      if (invalidSignatures.length) {
        ...
        invalidSignatures.map(i =>
          output.standard(`${color.stdout('red', `${i.name}@${i.version}`)} (${i.registry})`)
        )
        output.standard()
      }
