...
// project's `allowScripts` field, depending on `--pin` and the currently
// installed versions.
//
// Denying always writes `"<name>": false`, regardless of `--pin`, per the
...
const versionedKeyFor = (node) => {
  ...
  if (/^https?:\/\//.test(resolved)) {
    ...
    return trusted && trusted.version ? `${trusted.name}@${trusted.version}` : null
  }
...
const keyTargetsNode = (key, node) => {
  ...
  switch (parsed.type) {
    ...
    case 'range':
    case 'version': {
      // Compare against the URL-derived trusted name, never the tarball's
      // self-reported name.
      const trusted = getTrustedRegistryIdentity(node)
      const name = trusted ? trusted.name : node.name
