...
const getJsonError = (er, npm, { summary, detail }) => {
...
const errorMessage = (er, npm) => {
  ...
  const parsedFiles = files.map(([relativeFilePath, fileContent]) => {
    const filePath = `${npm.logPath}${relativeFilePath}`
    const content = `'Log files:\n${npm.logFiles.join('\n')}\n\n${fileContent.trim()}\n`

    // Add a detail item with the full path of each file written
  ...
  return {
    ...
    detail,
    files: parsedFiles,
    json: getJsonError(er, npm, { summary: short, detail }),
  }
}
