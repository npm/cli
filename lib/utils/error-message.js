    ...
    case 'E401':
      ...
      if (er.code === 'EOTP' || /one-time pass/.test(er.message)) {
        ...
        if (auth.includes('Bearer')) {
          ...
          const registry = npm?.loaded && npm.config.get('registry')
          // A custom registry has no `/forgot` page, so point at the root of
          // the configured registry instead. Fall back to the npmjs.com link
          // when npm is not loaded or the default registry is in use.
          // @see https://github.com/npm/cli/issues/10014
          const forgotUrl = !registry || registry === 'https://registry.npmjs.org/'
