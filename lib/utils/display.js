...
const redactStrings = (obj) => {
  ...
  if (isPlainObject(obj)) {
    return Object.fromEntries(
      Object.entries(obj).map(([key, value]) => [key, redactStrings(value)])
    )
  }
  return obj
}

// Redact each string value on its own. Redacting the serialized document
// instead lets a url inside one value match across the json around it and
// corrupt the output so it no longer parses.
