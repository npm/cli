...
// Translates log levels to styleText colors. `c` is a colorizer with the
// signature `(format, text) => string`.
const COLOR_PALETTE = (c) => ({
  heading: (text) => c('bold', text),
  title: (text) => c('blueBright', text),
  timing: (text) => c('magentaBright', text),
  // loglevels
  error: (text) => c('red', text),
  warn: (text) => c('yellow', text),
  notice: (text) => c('cyanBright', text),
  http: (text) => c('green', text),
  info: (text) => c('cyan', text),
  verbose: (text) => c('blue', text),
  silly: (text) => c(['blue', 'dim'], text),
})
...
class Display {
  ...
  async load ({
    ...
    color.setColor({ stdout: stdoutColor, stderr: stderrColor })
    this.#logColors = COLOR_PALETTE(color.colorize(stderrColor))
  ...
  #tryWriteLog (level, meta, ...args) {
    try {
      ...
      if (level === log.KEYS.warn && heading === 'ERESOLVE' && expl && typeof expl === 'object') {
        ...
        this.#writeLog(level, meta, '', explain(expl, color.colorize(this.#stderrColor), 2))
        return
      }
