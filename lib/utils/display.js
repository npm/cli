...
class Display {
  ...
  async load ({
    ...
    const chalk = stdoutColor ? new Chalk({ level }) : this.#noColorChalk
    const logChalk = stderrColor ? new Chalk({ level }) : this.#noColorChalk

    this.#stdoutColor = stdoutColor
    this.#stdoutChalk = chalk

    this.#stderrColor = stderrColor
    this.#stderrChalk = logChalk
    ...
    // Emit resume and flush events to flush all buffered output
    log.resume()
    output.flush()
  ...
  #outputHandler = withMeta((level, meta, ...args) => {
    switch (level) {
      case output.KEYS.flush: {
        ...
        if (meta.callback) {
          this.#stderr.write('', () => this.#stdout.write('', meta.callback))
        }
        break
      }
