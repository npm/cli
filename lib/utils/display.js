  ...
  #inputHandler = withMeta((level, meta, ...args) => {
    switch (level) {
      ...
      case input.KEYS.end:
        ...
        // After silent prompts, add newline to preserve output and move cursor to next line
        if (meta.silentPrompt) {
          output.standard('')
        }
        break
      ...
      case input.KEYS.read: {
        ...
        return input.start(() => p()
          // User pressed Enter, resolve the promise normally
          .then(res)
          // User pressed ctrl+c, reject the promise and add newline to preserve output
          .catch((error) => {
            rej(error)
            output.standard('')
            throw error
