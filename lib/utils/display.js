  ...
  #inputHandler = withMeta((level, meta, ...args) => {
    switch (level) {
      ...
      case input.KEYS.end: {
        ...
        const [options] = args
        const isSilentPrompt = Array.isArray(options) && options[0]?.silent
        if (isSilentPrompt) {
          output.standard('')
        }
        output.flush()
      ...
      case input.KEYS.read: {
        // The convention when calling input.read is to pass in a single fn that returns the promise to await. Resolve and reject are provided by proc-log. The last argument are the options passed to read which include the silent flag.
