...
class Progress {
  ...
  // Static flag to ensure listeners are only added once
  static #listenersAdded = false
  static #anyCursorHidden = false

  static #globalExitHandler = () => {
    // Show the cursor if any Progress instance hid it
    if (Progress.#anyCursorHidden) {
      try {
        process.stderr.write('\x1B[?25h')
        Progress.#anyCursorHidden = false
      } catch (e) {
        // eslint-disable-next-line no-console
        console.error('Failed to restore cursor visibility:', e)
      }
    }
  }
  ...
  constructor ({ stream }) {
    this.#stream = stream
    // Add exit/signal handlers only once globally
    if (!Progress.#listenersAdded) {
      process.on('SIGINT', Progress.#globalExitHandler)
      process.on('SIGTERM', Progress.#globalExitHandler)
      process.on('exit', Progress.#globalExitHandler)
      Progress.#listenersAdded = true
    }
  }
  ...
  off () {
    ...
    this.#lastUpdate = 0
    this.#clearSpinner()
    // Show the cursor again when spinner stops, but only if we hid it
    if (this.#cursorHidden) {
      this.#stream.write('\x1B[?25h')
      this.#cursorHidden = false
      Progress.#anyCursorHidden = false
    }
    // Do not remove global listeners here; they are shared
  }
  ...
  #renderSpinner () {
    ...
    // Hide the cursor when spinner is active
    if (!this.#cursorHidden) {
      this.#stream.write('\x1B[?25l')
      this.#cursorHidden = true
      Progress.#anyCursorHidden = true
    }
