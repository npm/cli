...
class Exec extends BaseCommand {
  ...
  async _exec (args) {
    ...
    const fetchManiForPackage = async pkg => {
      const spec = npa(pkg, this.npm.localPrefix)
      ...
      return await pacote.manifest(pkg, {
    ...
    // that fails.
    const manis = await Promise.all(packages.map(
      async p => normalize(await fetchManiForPackage(p))))
