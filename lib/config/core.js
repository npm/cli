...
Conf.prototype.save = async function (where) {
  const target = this.sources[where]
  if (!target || !(target.path || target.source) || !target.data) {
    if (where !== 'builtin') {
      throw new Error('bad save target: ' + where)
    }
    return this
  }
  ...
  if (target.source) {
    ...
    for (const k of Object.keys(target.data)) {
  ...
  if (!data.trim()) {
    return unlink(target.path).catch(() => this)
  }
  const dir = path.dirname(target.path)

  await mkdirp(dir)
  const st = await stat(dir)
  await writeFile(target.path, data, 'utf8')

  if (myUid === 0 && (myUid !== st.uid|| myGid !== st.gid)) {
    await chown(target.path, st.uid, st.gid)
  }
