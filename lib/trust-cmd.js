...
class TrustCommand extends BaseCommand {
  ...
  // Log a notice message with blue formatting
  notice (strings, ...values) {
    log.notice('trust', this.warnString(strings, ...values))
  }
  ...
  async createConfigCommand ({ positionalArgs, flags }) {
    ...
    this.notice`Establishing trust between ${options.values.package} package and ${providerName}`
    this.notice`Anyone with ${providerEntity} write access can publish to ${options.values.package}`
    this.notice`Two-factor authentication is required for this operation`
    if (!this.registryIsDefault) {
    ...
    this.notice`Trust configuration created successfully with the following settings:`
    const values = this.constructor.bodyToOptions(body)
    this.logOptions({ values })
  }
