...
class TrustCommand extends BaseCommand {
  ...
  warnString (strings, ...values) {
    const message = strings.reduce((result, str, i) => {
      return result + color.stderr('blue', str) + (values[i] ? color.stderr('reset', values[i]) : '')
    }, '')
    return message
  }
  ...
  logOptions (options, pad = true) {
    ...
    if (values) {
      const lines = []
      if (type) {
        lines.push(`type: ${color.stdout('green', type)}`)
      }
      if (id) {
        lines.push(`id: ${color.stdout('green', id)}`)
      }
      for (const [key, value] of Object.entries(rest)) {
        if (value !== null && value !== undefined) {
          const parts = [
            `${color.stdout('reset', key)}: ${color.stdout('green', value)}`,
          ]
          if (fromPackageJson && fromPackageJson[key]) {
            parts.push(`(${color.stdout('yellow', `from package.json`)})`)
          }
      ...
      if (formattedPermissions) {
        lines.push(`${color.stdout('reset', 'permissions')}: ${color.stdout('green', formattedPermissions)}`)
      }
      if (pad) {
        output.standard()
      }
      ...
      if (urls) {
        ...
        for (const key of Object.keys(rest)) {
          if (urls[key]) {
            urlLines.push(color.stdout('blue', urls[key]))
          }
        }
