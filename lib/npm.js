...
class Npm {
  ...
  constructor ({ npmRoot, argv, localPrefixRoot }) {
    this.#npmRoot = npmRoot
    this.config = new Config({ builtinRoot: npmRoot, argv, localPrefixRoot })
  ...
  async exec (commandName = this.#argvCommand, args = this.#argvArgs) {
    const runNotifier = !this.#command

    const Command = Npm.cmd(commandName)
