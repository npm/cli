...
class Npm {
  ...
  async exec (commandName = this.#argvCommand, args = this.#argvArgs) {
    const Impl = Npm.cmd(commandName)
    const command = new Impl(this)

    // exec can be re-entered by an already running command but if it is the
    // first time we want to kickoff the update notifier and set the command
    if (!this.#command) {
    ...
    return this.time(`command:${command.name}`, () => command.cmdExec(args))
  }
