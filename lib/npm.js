...
class Npm {
  ...
  #getError (rawErr, opts) {
    const { getError, getExitCodeFromError } = require('./utils/error-message.js')
    let formatted
    try {
      formatted = getError(rawErr, { npm: this, command: this.#command, ...opts })
    } catch (formatErr) {
      // A crashing formatter would otherwise exit without printing the original error.
      log.verbose('error-message', replaceInfo(formatErr.stack))
      formatted = {
        exitCode: getExitCodeFromError(rawErr) || 1,
        suppressError: false,
        summary: [['', replaceInfo(rawErr.message)]],
        error: rawErr.code ? [['code', rawErr.code]] : [],
      }
    }
    const { files = [], ...error } = formatted
