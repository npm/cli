...
class Npm {
  ...
  async exec () {
    ...
    if (typeof this.updateNotification === 'string') {
      log.notice('', this.updateNotification, { [META]: true, force: true })
    }
    ...
    log.verbose('npm ', 'v' + this.version)

    if (err && this.config.loaded && this.config.get('json')) {
      output.flush({
        [META]: true,
        jsonError: {
          code: err.code,
          summary: (err.summary || []).map(line => line.slice(1).join(' ')).join('\n'),
          detail: (err.detail || []).map(line => line.slice(1).join(' ')).join('\n'),
        },
      })
    }
