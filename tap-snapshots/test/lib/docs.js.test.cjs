...
\`npm deny-scripts\`, which uses a source-appropriate trusted identity
regardless of this setting: a name-only key for registry packages, the exact
source for direct remote and file dependencies, and the hosted repository
shortcut without a committish for hosted git dependencies.
